...
def render_login_screen(window):
    ...
    def on_login():
        ...
        if result:
            tk.Label(window, text='You are loged', fg='red').grid(row=4, column=1)
        else:
            tk.Label(window, text='Invalid registered!', fg='red').grid(row=4, column=1)
    ...
    tk.Button(window, text='Login',
              ...
              ).grid(row=2, column=1)  # grida go slaga na mqstoto
